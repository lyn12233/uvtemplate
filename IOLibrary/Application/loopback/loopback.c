...
int32_t loopback_tcps(uint8_t sn, uint8_t *buf, uint16_t port) {
  ...
  case SOCK_ESTABLISHED:
    if (getSn_IR(sn) & Sn_IR_CON) {
      ...
      printf("%d:Connected - %d.%d.%d.%d : %d\r\n", sn, destip[0], destip[1],
             destip[2], destip[3], destport);
#endif
...
int32_t loopback_tcpc(uint8_t sn, uint8_t *buf, uint8_t *destip,
                      uint16_t destport) {
  ...
  // Check the W5500 Socket n status register (Sn_SR, The 'Sn_SR' controlled by
  // Sn_CR command or Packet send/recv status)
  switch (getSn_SR(sn)) {
  case SOCK_ESTABLISHED:
    if (getSn_IR(sn) &
        Sn_IR_CON) { // Socket n interrupt register mask; TCP CON interrupt =
                     // connection with peer is successful
#ifdef _LOOPBACK_DEBUG_
      ...
      setSn_IR(
          sn,
          Sn_IR_CON); // this interrupt should be write the bit cleared to '1'
    }

    //////////////////////////////////////////////////////////////////////////////////////////////
    // Data Transaction Parts; Handle the [data receive and send] process
    //////////////////////////////////////////////////////////////////////////////////////////////
    if ((size = getSn_RX_RSR(sn)) > 0) { // Sn_RX_RSR: Socket n Received Size
                                         // Register, Receiving data length
      if (size > DATA_BUF_SIZE) {
        size = DATA_BUF_SIZE; // DATA_BUF_SIZE means user defined buffer size
                              // (array)
      }
      ret = recv(
          sn, buf,
          size); // Data Receive process (H/W Rx socket buffer -> User's buffer)

      if (ret <= 0) {
        return ret; // If the received data length <= 0, receive failed and
                    // process end
      }
      size = (uint16_t)ret;
      sentsize = 0;
      ...
      while (size != sentsize) {
        ret =
            send(sn, buf + sentsize,
                 size - sentsize); // Data send process (User's buffer ->
                                   // Destination through H/W Tx socket buffer)
  ...
  case SOCK_INIT:
    ...
    printf("%d:Try to connect to the %d.%d.%d.%d : %d\r\n", sn, destip[0],
           destip[1], destip[2], destip[3], destport);
#endif
...
int8_t check_loopback_mode_W6x00() {
  if (loopback_mode != AS_IPV4 && loopback_mode != AS_IPV6 &&
      loopback_mode != AS_IPDUAL) {
  ...
  case SOCK_ESTABLISHED:
    ...
    if (inter & Sn_IR_CON) {
      ...
      if (ext_status & TCPSOCK_MODE) {
        // IPv6
        printf("%d:Peer IP : %04X:%04X", sn,
               ((uint16_t)dst_ip[0] << 8) | ((uint16_t)dst_ip[1]),
        ...
        printf(":%04X:%04X, ",
               ((uint16_t)dst_ip[12] << 8) | ((uint16_t)dst_ip[13]),
               ((uint16_t)dst_ip[14] << 8) | ((uint16_t)dst_ip[15]));
      } else {
        // IPv4
        // getSn_DIPR(sn,dst_ip);
        printf("%d:Peer IP : %.3d.%.3d.%.3d.%.3d, ", sn, dst_ip[0], dst_ip[1],
               dst_ip[2], dst_ip[3]);
      }
    ...
    if (received_size > 0) {
      ...
      if (sn_status & TCPSOCK_MODE) {
        printf("Socket %d Received %d bytes from "
               "%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:"
               "%02x%02x port %d : \r\n",
               sn, received_size, destip[0], destip[1], destip[2], destip[3],
               destip[4], destip[5], destip[6], destip[7], destip[8], destip[9],
               destip[10], destip[11], destip[12], destip[13], destip[14],
               destip[15], destport);
      } else {
...
#ifdef _LOOPBACK_DEBUG_
      if (sn_status & TCPSOCK_MODE) {
        printf("Socket %d Sent back %d bytes from "
               "%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:"
               "%02x%02x port %d : \r\n",
               sn, sentsize, destip[0], destip[1], destip[2], destip[3],
               destip[4], destip[5], destip[6], destip[7], destip[8], destip[9],
               destip[10], destip[11], destip[12], destip[13], destip[14],
               destip[15], destport);
      } else {
...
int32_t loopback_tcpc(uint8_t sn, uint8_t *buf, uint8_t *destip,
  ...
  // Socket Status Transitions
  // Check the W6100 Socket n status register (Sn_SR, The 'Sn_SR' controlled by
  // Sn_CR command or Packet send/recv status)
  getsockopt(sn, SO_STATUS, &status);
  switch (status) {
  case SOCK_ESTABLISHED:
    ctlsocket(sn, CS_GET_INTERRUPT, &inter);
    if (inter & Sn_IR_CON) { // Socket n interrupt register mask; TCP CON
                             // interrupt = connection with peer is successful
#ifdef _LOOPBACK_DEBUG_
#if 1
      // 20231018 taylor
      getsockopt(sn, SO_EXTSTATUS, &sn_status);
      if (sn_status & TCPSOCK_MODE) {
        printf("%d:Connected to - "
               "%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:"
               "%02x%02x : %d\r\n",
               sn, destip[0], destip[1], destip[2], destip[3], destip[4],
               destip[5], destip[6], destip[7], destip[8], destip[9],
               destip[10], destip[11], destip[12], destip[13], destip[14],
               destip[15], destport);
      } else {
        printf("%d:Connected to - %d.%d.%d.%d : %d\r\n", sn, destip[0],
      ...
      ctlsocket(
          sn, CS_CLR_INTERRUPT,
          &arg_tmp8); // this interrupt should be write the bit cleared to '1'
    }
    ...
    if (received_size > 0) { // Sn_RX_RSR: Socket n Received Size Register,
                             // Receiving data length
      if (received_size > DATA_BUF_SIZE) {
        received_size = DATA_BUF_SIZE; // DATA_BUF_SIZE means user defined
                                       // buffer size (array)
      }
      ret = recv(sn, buf, received_size); // Data Receive process (H/W Rx socket
                                          // buffer -> User's buffer)
      ...
      // Data sentsize control
      while (received_size != sentsize) {
        ret = send(sn, buf + sentsize,
                   received_size -
                       sentsize); // Data send process (User's buffer ->
                                  // Destination through H/W Tx socket buffer)
        if (ret < 0) {            // Send Error occurred (sent data length < 0)
          close(sn);              // socket close
          return ret;
  ...
  case SOCK_CLOSE_WAIT:
    ...
    if ((received_size = getSn_RX_RSR(sn)) >
        0) { // Sn_RX_RSR: Socket n Received Size Register, Receiving data
             // length
      if (received_size > DATA_BUF_SIZE) {
        received_size = DATA_BUF_SIZE; // DATA_BUF_SIZE means user defined
                                       // buffer size (array)
  ...
  case SOCK_INIT:
    ...
    if (loopback_mode == AS_IPV4) {
      ...
      printf("%d:Try to connect to the %04X:%04X", sn,
             ((uint16_t)destip[0] << 8) | ((uint16_t)destip[1]),
    ...
    if (loopback_mode == AS_IPV4) {
      ret = connect(
          sn, destip, destport,
          4); /* Try to connect to TCP server(Socket, DestIP, DestPort) */
    } else if (loopback_mode == AS_IPV6) {
      ret = connect(
          sn, destip, destport,
          16); /* Try to connect to TCP server(Socket, DestIP, DestPort) */
    }
...
int32_t loopback_udps(uint8_t sn, uint8_t *buf, uint16_t port) {
  ...
  case SOCK_UDP:
    ...
    if (received_size > 0) {
      ret = recvfrom(sn, buf, received_size, (uint8_t *)&destip,
                     (uint16_t *)&destport, &addr_len);
      ...
      while (sentsize != received_size) {
        ret = sendto(sn, buf + sentsize, received_size - sentsize, destip,
                     destport, addr_len);
