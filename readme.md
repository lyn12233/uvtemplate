...
- testing on uvhelper: to gen stub file for hinting, see [here](./uvhelper/readme.md)
...
modes in arm instruction set:
    1. arm
    2. thumb: compact 16-bit mode(not available for cortex-m)

stm32 pin categories:
 - power
 - oscillator
 - reset/download/boot
 - gpio
...
required pins on board:
 - VCC: 3.3V
 - GND
 - NRF_CS/pg7: nrf chip select (low to select)(?or spi2_nss, pb12)
 - NRF_CE/pg8: nrf chip enable
 - NRF_IRQ/pg6: nrf interrupt request
 - SPI2_MOSI/pb15: (spi) master output, slave input
 - SPI2_MISO/pb14: (spi) master input, slave output
 - SPI2_SCK/pb13: (spi) synchronization clock

$$\begin{align*}
  & & & (\mathrm{W5500\;module}) & & &\\
  & \circ & & \square & & \circ& \\
  &\quad\mathrm{SCLK}\quad & \quad\mathrm{SCS}\quad & \quad\mathrm{MOSI}\quad & \quad\mathrm{MISO}\quad & \quad\mathrm{3V3}\quad & \\
  & \quad\mathrm{GND}\quad & \quad\mathrm{INT}\quad & \quad\mathrm{RST}\quad & \quad\mathrm{GND}\quad & \quad\mathrm{5V}\quad & \\
\end{align*}$$

transplantation requires board-dependent SPI read/write implementations, as well as other configs

**official transplantation TODO's**:
 - `_WIZCHIP_IO_BASE_` for bus interface, including options:
   - bus mode for non-spi(?): direct/indirect
   - spi mode: variable length(recommended)/ fixed length/ qspi
 - `_PHY_IO_MODE_` for ethernet PHYsical layer access mode, options:
   - PHYCR: access PHY state by control register
   - MILL:media independent interface, requires MDC/MDIO software(?)
 - `reg_wizchip_xxx_cbfunc()` to register spi-related callback functions
   - cris: entering/exiting critical section, e.g. disabling/enabling whole interrupt
   - cs: chip select
   - spi: writebyte and readbyte
   - spiburst: spi burst read/write; opt to use this; fallback if not defined(?)
...
- ./Task: implement freertos tasks
- ./User: optional initializers, etc
